...
{
	public readonly codecs: ICodecFamily<SharedTreeChange>; // TODO
	...
	public intoDelta(change: SharedTreeChange): Delta.Root {
		const map = new Map<FieldKey, Delta.MarkList>();
		if (change.modularChange !== undefined) {
			for (const [field, mark] of this.modularChangeFamily.intoDelta(change.modularChange)) {
				map.set(field, mark);
			}
		}
		if (change.schemaChange !== undefined) {
			for (const [field, mark] of this.schemaChangeFamily.intoDelta(change.schemaChange)) {
				map.set(field, mark);
			}
		}
		return map;
